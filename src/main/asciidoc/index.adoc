...
The link:jsdoc/vert_options-VertxOptions.html[`VertxOptions`] object has many settings and allows you to configure things like clustering,
...
link:jsdoc/vert_options-VertxOptions.html[`VertxOptions`] object before creating the Vertx object.
...
If you want to deploy a verticle as a worker verticle you do that with link:jsdoc/deploymen_options-DeploymentOptions.html#setWorker[`setWorker`].
...
The verticle name is used to look up the specific link:jsdoc/verticl_factory-VerticleFactory.html[`VerticleFactory`] that will be used to
...
Extra classpath entries can also be provided with link:jsdoc/deploymen_options-DeploymentOptions.html#setExtraClasspath[`setExtraClasspath`] so they
