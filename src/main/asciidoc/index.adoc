...
The link:jsdoc/vertx_options-VertxOptions.html[`VertxOptions`] object has many settings and allows you to configure things like clustering,
...
link:jsdoc/vertx_options-VertxOptions.html[`VertxOptions`] object before creating the Vertx object.
...
If you want to deploy a verticle as a worker verticle you do that with link:jsdoc/deployment_options-DeploymentOptions.html#setWorker[`setWorker`].
...
The verticle name is used to look up the specific link:jsdoc/verticle_factory-VerticleFactory.html[`VerticleFactory`] that will be used to
...
This configuration is then available via the link:jsdoc/context-Context.html[`Context`] object.
...
Extra classpath entries can also be provided with link:jsdoc/deployment_options-DeploymentOptions.html#setExtraClasspath[`setExtraClasspath`] so they
