...
Use `link:../../jsdoc/http_server_request-HttpServerRequest.html#host[host]` to return the host of the HTTP request.
...
To receive unknown frames, you can use the `link:../../jsdoc/http_server_request-HttpServerRequest.html#unknownFrameHandler[unknownFrameHandler]` on the request,
this will get called every time an unknown frame arrives. Here's an example:

[source,js]
----

request.unknownFrameHandler(function (frame) {
...
`link:../../jsdoc/http_server_request-HttpServerRequest.html#rawMethod[rawMethod]` returns the HTTP method as sent by the client.
...
To send such frames, you can use the `link:../../jsdoc/http_server_response-HttpServerResponse.html#writeFrame[writeFrame]` on the response.
Here's an example:
...
The request handler are notified of stream reset events with the `link:../../jsdoc/http_server_request-HttpServerRequest.html#exceptionHandler[request handler]` and
`link:../../jsdoc/http_server_response-HttpServerResponse.html#exceptionHandler[response handler]`:
...
The `link:../../jsdoc/http_server_response-HttpServerResponse.html#push[push]` method must be called before the initiating response ends, however
...
`link:../dataobjects.html#HttpClientOptions#setH2cUpgrade[h2cUpgrade]` options is set to false: after the
...
with `link:../../jsdoc/http_client_response-HttpClientResponse.html#version[version]` when the response arrives.
...
is used, `link:../../jsdoc/http_client_request-HttpClientRequest.html#setRawMethod[setRawMethod]` must be used to
...
To send such frames, you can use the `link:../../jsdoc/http_client_request-HttpClientRequest.html#write[write]` on the request. Here's an example:
...
The request handler are notified of stream reset events with the `link:../../jsdoc/http_client_request-HttpClientRequest.html#exceptionHandler[request handler]` and
`link:../../jsdoc/http_client_response-HttpClientResponse.html#exceptionHandler[response handler]`:
...
[source,js]
----
response.unknownFrameHandler(function (frame) {
...
HTTP/2 connections will not be closed by the client automatically. To close them you can call `link:../../jsdoc/http_connection-HttpConnection.html#close[close]`
...
The `link:../../jsdoc/http_connection-HttpConnection.html[HttpConnection]` offers the API for dealing with HTTP/2 connection events, lifecycle
and settings.

==== Server connections

The `link:../../jsdoc/http_server_request-HttpServerRequest.html#connection[connection]` method returns the request connection on the server:
...
The `link:../../jsdoc/http_client_request-HttpClientRequest.html#connection[connection]` method returns the request connection on the client:
...
Conversely the `link:../../jsdoc/http_connection-HttpConnection.html#remoteSettingsHandler[remoteSettingsHandler]` is notified
...
validity: `link:../../jsdoc/http_connection-HttpConnection.html#ping[ping]` sends a `PING` frame to the remote
endpoint:
...
Calling `link:../../jsdoc/http_connection-HttpConnection.html#shutdown[shutdown]` will send a `GOAWAY` frame to the
...
Connection `link:../../jsdoc/http_connection-HttpConnection.html#close[close]` close is a shutdown with no delay, the `GOAWAY`
frame will still be sent before the connection is closed.

The `link:../../jsdoc/http_connection-HttpConnection.html#closeHandler[closeHandler]` notifies when connection is closed,
`link:../../jsdoc/http_connection-HttpConnection.html#shutdownHandler[shutdownHandler]` notifies when all streams have been closed but the
...
The `link:../../jsdoc/http_connection-HttpConnection.html#shutdownHandler[shutdownHandler]` will be called when all current streams
...
`link:../../jsdoc/server_web_socket-ServerWebSocket.html#path[path]`, `link:../../jsdoc/server_web_socket-ServerWebSocket.html#query[query]` and
`link:../../jsdoc/server_web_socket-ServerWebSocket.html#uri[URI]` of the HTTP request of the WebSocket handshake.
