...
When using `link:../../jsdoc/event_bus-EventBus.html#send[send]` the event bus attempts to deliver the message to a
`link:../../jsdoc/message_consumer-MessageConsumer.html[MessageConsumer]` registered with the event bus.

In some cases it's useful for the sender to know when the consumer has received the message and "processed" it.

To acknowledge that the message has been processed the consumer can reply to the message by calling `link:../../jsdoc/message-Message.html#reply[reply]`.
