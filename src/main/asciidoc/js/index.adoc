...
If you want to deploy a verticle as a worker verticle you do that with `link:../cheatsheet/DeploymentOptions.html#setWorker[setWorker]`.
...
Extra classpath entries can also be provided with `link:../cheatsheet/DeploymentOptions.html#setExtraClasspath[setExtraClasspath]` so they
...
`link:jsdoc/vertx_metrics-VertxMetrics.html[API Documentation]`.
