...
You can't do much in Vert.x-land unless you can commune with a `link:../../jsdoc/vertx-Vertx.html[Vertx]` object!
...
It's done by calling `link:../../jsdoc/vertx-Vertx.html#executeBlocking[executeBlocking]` specifying both the blocking code to execute and a
...
If you don't care about ordering you can call `link:../../jsdoc/vertx-Vertx.html#executeBlocking[executeBlocking]`
...
----
var someAPI = new (Java.type("examples.CoreExamples.BlockingAPI"))();
var executor = vertx.createWorkerExecutor("my-worker-pool");
...
var executor = vertx.createWorkerExecutor("my-worker-pool", poolSize, maxExecuteTime);
...
Coordination of multiple asynchronous results can be achieved with Vert.x `link:../../jsdoc/future-Future.html[futures]`.

`link:../../jsdoc/composite_future-CompositeFuture.html#all[CompositeFuture.all]` takes several futures arguments (up to 6) and returns a future that is
...
The handler returned by `link:../../jsdoc/future-Future.html#completer[completer]` completes the future.

`link:../../jsdoc/composite_future-CompositeFuture.html#any[CompositeFuture.any]` takes several futures arguments (up to 6) and returns a future that is
...
`link:../../jsdoc/future-Future.html#compose[compose]` can be used for chaining futures:
...
You can deploy a verticle using one of the `link:../../jsdoc/vertx-Vertx.html#deployVerticle[deployVerticle]` method, specifying a verticle
...
You can also programmatically register and unregister verticle factories using `link:../../jsdoc/vertx-Vertx.html#registerVerticleFactory[registerVerticleFactory]`
and `link:../../jsdoc/vertx-Vertx.html#unregisterVerticleFactory[unregisterVerticleFactory]` if you wish.
...
Deployments can be undeployed with `link:../../jsdoc/vertx-Vertx.html#undeploy[undeploy]`.
...
If you are embedding Vert.x and you have finished with it, you can call `link:../../jsdoc/vertx-Vertx.html#close[close]` to close it
down.
...
To retrieve the context, use the `link:../../jsdoc/vertx-Vertx.html#getOrCreateContext[getOrCreateContext]` method:
...
`link:../../jsdoc/context-Context.html#runOnContext[runOnContext]`:
...
The context object also let you access verticle configuration using the `link:../../jsdoc/context-Context.html#config[config]`
...
To set a timer to fire once you use `link:../../jsdoc/vertx-Vertx.html#setTimer[setTimer]` method passing in the delay and a handler
...
You can also set a timer to fire periodically by using `link:../../jsdoc/vertx-Vertx.html#setPeriodic[setPeriodic]`.
...
In this case, you should consider using `link:../../jsdoc/vertx-Vertx.html#setTimer[setTimer]` instead. Once your treatment has
...
To cancel a periodic timer, call `link:../../jsdoc/vertx-Vertx.html#cancelTimer[cancelTimer]` specifying the timer id. For example:
...
Verticle use the Vert.x worker pool for executing blocking actions, i.e `link:../../jsdoc/context-Context.html#executeBlocking[executeBlocking]` or
worker verticle.
...
For example if you have deployed a verticle which creates a `link:../../jsdoc/net_server-NetServer.html[NetServer]` which provides
`link:../../jsdoc/net_socket-NetSocket.html[NetSocket]` instances in it's handler, then it's best to always access that socket instance
...
DNS servers can be also configured when creating a `link:../../jsdoc/vertx-Vertx.html[Vertx]` instance:
