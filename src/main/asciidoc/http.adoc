...
If you don't want the default, a server can be configured by passing in a link:dataobject/HttpServerOptions.html[`HttpServerOptions`]
...
closed after an idle time, then you configure link:dataobject/HttpServerOptions.html#setIdleTimeout[`setIdleTimeout`].
...
To enable compression use can configure it with link:dataobject/HttpServerOptions.html#setCompressionSupported[`setCompressionSupported`].
...
To enable compression set link:dataobject/HttpClientOptions.html#setTryUseCompression[`setTryUseCompression`] on the options
...
For pooling to work, keep alive must be true using link:dataobject/HttpClientOptions.html#setKeepAlive[`setKeepAlive`]
...
The maximum number of connections to pool *for each server* is configured using link:dataobject/HttpClientOptions.html#setMaxPoolSize[`setMaxPoolSize`]
...
To enable pipe-lining, it must be enabled using link:dataobject/HttpClientOptions.html#setPipelining[`setPipelining`].
...
link:dataobject/HttpClientOptions.html#setMaxWebsocketFrameSize[`setMaxWebsocketFrameSize`]
